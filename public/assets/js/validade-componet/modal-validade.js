...
document.addEventListener("DOMContentLoaded", function () {
  Promise.all([
    fetch(
      "https://controle-de-estoque-ccb.onrender.com/api/validades?dias=30"
    ).then((res) => res.json()),
    fetch(
      "https://controle-de-estoque-ccb.onrender.com/api/validades-limpeza?dias=30"
    ).then((res) => res.json()),
  ])
    .then(([produtos, limpeza]) => {
      let setores = [];

      if (produtos.length > 0) setores.push('"Produtos"');
      if (limpeza.length > 0) setores.push('"Limpeza"');

      if (setores.length > 0) {
        mostrarNotificacaoValidades(setores);
      }
    })
    .catch((error) => console.error("Erro ao buscar validades:", error));
});

function mostrarNotificacaoValidades(setores) {
  const container = document.querySelector(".notifications-container");

  // Impede múltiplas notificações iguais
  if (document.getElementById("notificacao-validade")) return;

  const setoresMsg = setores.join(" e ");
    ...
    <div class="flex">
      ...
      <div class="alert-prompt-wrap">
        <p class="text-sm text-yellow-700">
          Temos algumas validades do ${setoresMsg} que estão para vencer! Vá em menu para verificar.
        </p>
      </div>
    </div>
  `;
